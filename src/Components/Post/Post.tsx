...
export default function Post({ user, post }: PostProps) {
  ...
  return (
    <>
      {deleteClicked && (<div className="w-full flex items-center justify-center  px-2 h-screen z-50 bg-black bg-opacity-50 fixed top-0 left-0">
            <div className="w-full max-w-lg flex flex-col gap-8 items-center opacity-100 py-8 bg-slate-900 rounded-md px-4 ">
              <p>Are you sure you want to delete this post? if you delete it, you won't be able to get it back</p>
      ...
      <div className="rounded-md pb-4 bg-slate-950 w-full max-w-lg mx-auto w-fullshadow-xl mb-4">
        <div className="flex gap-4 items-center pt-4 pl-4">
          ...
          <div className={`dropdown dropdown-end ml-auto mr-4 ${deleteClicked && "hidden"}`}>
            ...
            >
              {post.user_id === auth.currentUser?.uid && <li onClick={()=> setDeleteClicked(true)}><a>Delete</a></li>}
              <li>
                <a>Hide</a>
              </li>
              <li>
