...
import { addReply, getCommentReplies } from "../../Firebase/api/database/RepliesApi";
...
export default function RepliesContent({ commentID }: { commentID: string }) {
    ...
    const scheme = z.object({
        replyText: string().min(1, { message: "comment text can't be empty" }),
    });
        ...
        <div className="px-4">
          <div className="flex gap-2 mt-2">
            ...
            <form
              ...
              className="w-full bg-slate-900 h-10 rounded-md flex"
            >
              <textarea
                {...register("replyText")}
          ...
          {isLoading && (
            <div role="status" className="w-full mt-4 flex flex-col items-center">
              <svg
                aria-hidden="true"
                className="inline w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-yellow-400"
                viewBox="0 0 100 101"
          ...
          {data?.length && data?.length > 0 ? (
            data?.map((item) => <ReplyContentBody key={item?.created_at} reply={item} />)
          ) : (
            <div className="w-full flex items-center justify-center mt-2">
              <p className="mx-auto">no replies yet</p>
            </div>
          )}
        </div>
      );  
