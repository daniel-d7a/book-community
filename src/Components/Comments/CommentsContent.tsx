...
export default function CommentsContent({ postID }: { postID: string }) {
  ...
  const scheme = z.object({
    commText: string().min(1, { message: "comment text can't be empty" }),
  });
  const {
    register,
    handleSubmit,
  ...
  return (
    <div className="px-4">
      ...
      {isLoading && <div role="status" className="w-full mt-4 flex flex-col items-center">
        <svg
          aria-hidden="true"
          className="inline w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-yellow-400"
          viewBox="0 0 100 101"
      ...
      {data?.length > 0 ? (
        data?.sort(function(a,b){return b.created_at - a.created_at}).map((item) => <CommentContentBody key={item.id} comment={item} />)
      ) : (
        <div className="w-full flex items-center justify-center mt-2">
          <p className="mx-auto">no comments yet</p>
        </div>
      )}
