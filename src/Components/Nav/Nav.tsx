            ...
            {inputFile.current?.value && <div className='w-full flex items-center justify-center  px-2 h-screen z-50 bg-black bg-opacity-50 fixed top-0 left-0'>
                    <div className='w-full max-w-lg flex flex-col gap-8 items-center opacity-100 py-8 bg-slate-900 rounded-md px-4'>
                        ...
                        <div className='flex gap-4'>
                            <button className='p-2 px-4 rounded-md bg-yellow-500' onClick={(e) => {
                                console.log("clicked")
                                mutate()
                                e.target.disabled = true}}>Save</button>
                            <button className='p-2 px-4 rounded-md bg-yellow-950 text-yellow-500' onClick={onButtonClick}>Select other photo</button>
                            <button className='p-2 px-4 rounded-md text-red-500 bg-red-950 bg-opacity-50' onClick={() => {
            ...
            <div className="navbar justify-between bg-slate-950 fixed top-0 mb-4 z-30">
                ...
                <div className="flex-none">
                    ...
                    <div onClick={()=>setTools(!tools)} className='w-9 h-9 flex items-center justify-center rounded-sm text-lg bg-slate-900 overflow-hidden'>
                        <img src={data.profile_photo?data.profile_photo:"https://st3.depositphotos.com/6672868/13701/v/450/depositphotos_137014128-stock-illustration-user-profile-icon.jpg"}/>
                    </div>
                    <div className={`absolute top-14 right-3 w-60 p-4 bg-slate-950 rounded-md ${tools?"":"invisible"}`}>
                        <div className='flex flex-col gap-2'>
                            <div className='flex gap-4'>
                                <img onClick={() => setChangeProfile(!changeProfile)} src={data.profile_photo?data.profile_photo:"https://st3.depositphotos.com/6672868/13701/v/450/depositphotos_137014128-stock-illustration-user-profile-icon.jpg"} className=" w-12 h-12 rounded-full object-cover" />
                                {changeProfile && <button onClick={onButtonClick} className='z-40 absolute top-12 left-8 rounded-md bg-slate-950 p-2'>Change profile picture</button>}
                                <input onChange={onFileSelected} type='file' id='file' ref={inputFile} accept="image/png, image/jpeg" style={{display: 'none'}}/>
                                <div className='flex flex-col'>
                                    <p className='font-bold'>{data.username}</p>
                                    <p className='text-sm text-gray-400'>{data.type === 'r'? "Reader" : "Writer"}</p>
                                </div>
                            </div>
