    ...
    return (
      <div className="bg-slate-900 mb-0 overflow-hidden">
        ...
        <div className="px-4 pt-20">
          ...
          {status === "loading" && <div role="status" className="w-full h-screen flex flex-col pt-4 items-center">
          ...
          {status === "success" && data.map(
            (post) =>
              post.user_data && <Post key={post.id} user={post.user_data as SignUpData} post={post as ApiPost} />
          )}
        </div>
      </div>
    );
